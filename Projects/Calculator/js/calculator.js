...
const Calculator = {
    ...
    Display_Value: '0',
    // This will hold the first operand for any expressions, we'll set it to null for now.
    First_Operand: null,
...
function Handle_Operator(Next_Operator) {
    ...
    if (First_Operand == null) {
        ...
        let result = Perfrom_Calculation[operator](Value_Now, Value_of_Input);
...
keys.addEventListener('click', (event) => {
    ...
    // that was clicked.
    const { target } = event;
    // If the elemnet that was clicked on was not a button, exit the function.
